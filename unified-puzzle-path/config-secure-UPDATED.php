<?php
// Unified Puzzle Path Configuration - LIVE SITE VERSION (UPDATED FOR PP CODES)
...
// UPDATED: Secure helper function to extract hunt code from booking number (now supports PP codes)
function extractHuntCodeFromBooking($booking_number) {
    ...
    // Expected format: BB-YYYYMMDD-XXXX, EP-YYYYMMDD-XXXX, or PP-XXXXXX
    ...
    $valid_codes = ['BB', 'EP', 'BBR1', 'PP']; // ADDED PP support
...
// UPDATED: Secure mapping function with validation (now includes PP mapping)
function mapBookingCodeToHuntCode($booking_code) {
    ...
    $mapping = [
        ...
        'PP' => 'BBR1'    // ADDED: PP codes also map to BBR1 (default hunt)
    ];
