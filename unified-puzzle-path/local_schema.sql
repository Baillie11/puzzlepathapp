-- PuzzlePath Local Development Database Schema
-- Simplified schema for XAMPP/WordPress integration

-- Create basic users table (if not exists from WordPress)
...
-- Simplified booking table
CREATE TABLE IF NOT EXISTS pp_bookings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (hunt_id) REFERENCES pp_hunts(id)
);
...
ON DUPLICATE KEY UPDATE hunt_name = VALUES(hunt_name);
...
ON DUPLICATE KEY UPDATE medal_name = VALUES(medal_name);

-- Sample test booking
INSERT INTO pp_bookings (booking_code, hunt_id, participant_names, participant_count, booking_date, user_email, status) VALUES
('BB-20250826-TEST', 1, 'Test Player', 1, '2025-08-26', 'test@example.com', 'active'),
('EP-20250826-TEST', 2, 'Test Player', 1, '2025-08-26', 'test@example.com', 'active')
ON DUPLICATE KEY UPDATE booking_code = VALUES(booking_code);

-- Sample test user
INSERT INTO users (username, user_email, user_pass, display_name) VALUES
('testplayer', 'test@example.com', '$2y$10$DummyHashForTesting', 'Test Player')
