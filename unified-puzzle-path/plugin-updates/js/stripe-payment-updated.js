/**
 * Stripe Payment Processing for PuzzlePath Booking with Quest Integration
 * Version 1.3.0
 */
...
jQuery(document).ready(function ($) {
    ...
    var payWithCard = function(stripe, cardNumberElement, cardExpiryElement, cardCvcElement, postalCodeElement) {
        ...
        .then(function(result) {
            if (result.bookingCode) {
                ...
                bookingData.bookingCode = result.bookingCode;
                bookingData.huntCode = result.huntCode;
                bookingData.huntName = result.huntName;
                bookingData.isQuestEvent = result.isQuestEvent;
            }
    ...
    var showSuccessMessage = function() {
        ...
        // Add booking code to success message
        var codeMsg = bookingCode ? '<p><strong>Your booking code:</strong> <code style="background: #f1f1f1; padding: 4px 8px; border-radius: 4px; font-size: 1.1em;">' + bookingCode + '</code></p>' : '';
        $('#payment-success-message').append(codeMsg);
        
        // Show quest link if it's a quest event
        if (bookingData && bookingData.isQuestEvent && puzzlepath_data.unified_app_url) {
            showQuestLink();
        }
    };
    
    var showQuestLink = function() {
        var questContainer = $('#quest-link-container');
        var startButton = $('#start-quest-btn');
        
        // Set the quest URL with booking code as parameter
        var questUrl = puzzlepath_data.unified_app_url;
        if (questUrl && bookingCode) {
            // Add booking code as URL parameter for auto-fill
            if (questUrl.indexOf('?') !== -1) {
                questUrl += '&booking=' + encodeURIComponent(bookingCode);
            } else {
                questUrl += '?booking=' + encodeURIComponent(bookingCode);
            }
            startButton.attr('href', questUrl);
        }
        
        // Update the quest name in the message
        if (bookingData.huntName) {
            questContainer.find('h3').text('🎯 Ready to Start Your ' + bookingData.huntName + '?');
            questContainer.find('p').html('Your quest is ready! Click below to begin your adventure.<br><small>Your booking code will be automatically entered.</small>');
        }
        ...
        setTimeout(function() {
            questContainer.addClass('pulse-animation');
        }, 500);
    };
    
    // Add some CSS for better quest button styling
    var questCSS = `
        <style>
        .pulse-animation {
